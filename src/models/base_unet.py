...
from .attn.attend import Attend
...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, *, time_emb_dim: Optional[int] = None, groups=8):
        super().__init__()
        self.mlp = (
            nn.Sequential(
                nn.SiLU(), nn.Linear(time_emb_dim, dim_out * 2)  # type: ignore
            )
    ...
    def forward(self, x, time_emb=None):
        ...
        if exists(self.mlp) and exists(time_emb):
            time_emb = self.mlp(time_emb)  # type: ignore
